...
func (cli *DockerCli) CmdVolumes(args ...string) error {
	...
	description += "\nRun 'docker volumes COMMNAD --help' for more information on a command."

	cmd := cli.Subcmd("volumes", "[COMMAND]", description)
	if len(args) > 0 && (args[0] == "--help" || args[0] == "-h") {
		cmd.Usage()
		return nil
	}

	return cli.CmdVolumesLs(args...)
}

func (cli *DockerCli) CmdVolumesLs(args ...string) error {
	cmd := cli.Subcmd("volumes ls", "", "List volumes")
	...
	for _, out := range outs.Data {
		...
		if *size {
			fmt.Fprintf(w, fmt.Sprintf("%s\t", units.HumanSize(out.GetInt64("Size"))))
		}
		fmt.Fprintf(w, "\n")
	}
...
func (cli *DockerCli) CmdVolumesInspect(args ...string) error {
	cmd := cli.Subcmd("volumes inspect", "", "Inspect a volume")
...
func (cli *DockerCli) CmdVolumesRm(args ...string) error {
	cmd := cli.Subcmd("volumes rm", "", "remove a volume")
	if err := cmd.Parse(args); err != nil {
		return nil
	}
	if cmd.NArg() < 1 {
...
func (cli *DockerCli) CmdVolumesCreate(args ...string) error {
	var (
		cmd  = cli.Subcmd("volumes create", "", "create a volume")
